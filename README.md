...
These patterns extract and structure the thinking behind that approach. They come from a [production-grade Telegram trading bot](docs/references/reference-telegram-trading-bot-case-study.md) built fully agentically with Claude Code as the primary development tool. That project developed **Stack-First Development** — bringing up the entire application stack in Docker and testing through API endpoints only. Crucially, it treats the stack and the end-to-end functionality under test at a unit level: the strict conditions for a test case passing require the end-to-end functionality to be provably and fully functional.
